    ...
    public class ResourceMapWindow : EditorWindow
    {
        ...
        void DrawToolBar()
        {
            using (new EditorGUILayout.HorizontalScope(EditorStyles.toolbar))
            {
                ...
                {
                    Map.DependencyInfo.Load(false); 
                    CreateNode();
                }

                GUILayout.Space(4);

                if (GUILayout.Button("Load AB", EditorStyles.toolbarButton, GUILayout.Width(100)))
                {
                    Map.DependencyInfo.Load(true);
                    CreateNode();
                }
        ...
        void DrawNode(int id)
        {
            ...
            {
                Debug.Log("路径：" + node.Path);
                // var isLoopDep = Map.CheckIsLoopDependency(node.Path, node.Path);
            }
